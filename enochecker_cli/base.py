...
def _get_parser(hide_checker_address=False) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "method", choices=TASK_TYPES, help="One of {} ".format(TASK_TYPES)
    )
    ...
    parser.add_argument(
        "-r", "--round_id", type=int, default=1, help="The round we are in right now"
    )
    parser.add_argument(
        "-R",
        "--round_length",
...
def main():
    ...
    result = requests.post(
        ns.checker_address, data=msg, headers={"content-type": "application/json"},
    )
    if result.ok:
